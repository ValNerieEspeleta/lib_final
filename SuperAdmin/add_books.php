...
$setupSQL = [
    ...
    "CREATE TABLE IF NOT EXISTS lib_locations (id INT PRIMARY KEY AUTO_INCREMENT, location_name VARCHAR(255) NOT NULL UNIQUE)",
...
// Helper to safely add columns (MySQL 5.7+ compatibility, replaces IF NOT EXISTS)
...
safeAddColumn($conn, 'lib_authors', 'author_name', 'VARCHAR(255) AFTER id');
safeAddColumn($conn, 'lib_authors', 'first_name', 'VARCHAR(255) AFTER author_name');
safeAddColumn($conn, 'lib_authors', 'last_name', 'VARCHAR(255) AFTER first_name');

$columnsToAdd = [
    ['lib_books', 'author_id', 'INT'],
    ['lib_books', 'publisher_id', 'INT'],
    ['lib_books', 'isbn_id', 'INT'],
    ['lib_books', 'location', 'VARCHAR(255)'],
    ['lib_books', 'physical_description', 'VARCHAR(255)'],
    ['lib_books', 'date_receive', 'DATETIME'],
    ['lib_books', 'book_image', 'VARCHAR(255)']
];

foreach ($columnsToAdd as $col) {
    safeAddColumn($conn, $col[0], $col[1], $col[2]);
}
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $publisher_name = mysqli_real_escape_string($conn, isset($_POST['publisher_name']) ? $_POST['publisher_name'] : '');
    $place_of_pub = mysqli_real_escape_string($conn, isset($_POST['place_of_publication']) ? $_POST['place_of_publication'] : '');
    $location_name = mysqli_real_escape_string($conn, isset($_POST['location_name']) ? $_POST['location_name'] : '');
    $physical_desc = mysqli_real_escape_string($conn, isset($_POST['physical_description']) ? $_POST['physical_description'] : '');
    $isbn_number = mysqli_real_escape_string($conn, isset($_POST['isbn_number']) ? $_POST['isbn_number'] : '');
    $date_receive = !empty($_POST['date_receive']) ? mysqli_real_escape_string($conn, $_POST['date_receive']) : NULL;
    $accession_number = mysqli_real_escape_string($conn, isset($_POST['accession_number']) ? $_POST['accession_number'] : '');
    
    $acquisition_type = mysqli_real_escape_string($conn, isset($_POST['acquisition_type']) ? $_POST['acquisition_type'] : 'Purchased');
    $donor = mysqli_real_escape_string($conn, isset($_POST['donor']) ? $_POST['donor'] : '');
    ...
    if ($bookExists) {
        ...
        // but we keep the logic compatible.
        ...
        $query = "INSERT INTO lib_books (book_id, title, description, genre_id, status, date_created, author_id, publisher_id, location, physical_description, date_receive, book_image) 
                  VALUES ('$book_id', '$title', '$description', '$genre_id', '$status', '$date_created', " . ($main_author_id ?: "NULL") . ", " . ($publisher_id ?: "NULL") . ", '$location', '$physical', $dateStr, " . ($book_image ? "'$book_image'" : "NULL") . ")";
                ...
                <div class="card-body">
                  ...
                  <form class="forms-sample" method="POST" action="" enctype="multipart/form-data">
                    ...
                    <div class="form-group">
                      ...
                      <small class="text-muted">Upload a clear photo of the book cover.</small>
                    </div>
